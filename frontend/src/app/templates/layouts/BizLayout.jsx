      ...
      <div style={{ background: t.bg, borderBottom: `1px solid ${ac}20`, padding: "12px clamp(16px, 4vw, 48px)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          {[
            ...
            <a key={s.label} href={`#${s.id}`} onClick={(e) => handleScrollToSection(e, s.id)} style={{ fontSize: 13, color: fg, opacity: 0.5, textDecoration: "none", transition: "opacity 0.2s" }}
      ...
      <div style={{ padding: "0 clamp(16px, 5vw, 48px) clamp(40px, 8vw, 80px)", maxWidth: 896, margin: "0 auto", boxSizing: "border-box" }}>
        ...
        {p.skills?.length > 0 && (
          <ScrollReveal>
            <div style={{ marginBottom: 56 }}>
              ...
              <Tags items={p.skills} bg={`${ac}12`} fg={ac} border={`1px solid ${ac}30`} radius={radius} />
            </div>
          </ScrollReveal>
        )}
        ...
        {p.languages?.length > 0 && (
          <ScrollReveal>
            <div style={{ marginBottom: 56 }}>
              {lbl("Languages")}
              <Tags items={p.languages.map(l => `${l.name} (${l.proficiency})`)} bg={`${ac}08`} fg={fg} border={`1px solid ${ac}20`} radius={radius} />
            </div>
          </ScrollReveal>
        )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 16 }}>
                {p.projects.map((proj, i) => (
                  <div key={i} style={{
                    ...
                    {proj.image && (
                      <div style={{ overflow: "hidden", borderBottom: `1px solid ${ac}20` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 14 }}>
                {p.testimonials.map((tt, i) => (
                  <blockquote key={i} style={{
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = `${ac}18`;
                      e.currentTarget.style.transform = "none";
                      e.currentTarget.style.boxShadow = "none";
                    }}>
                    "{tt.quote}"
