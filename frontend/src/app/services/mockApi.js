...
export async function mockAIAssistant(prompt) {
  ...
  return {
    reply: `Here's an idea for "${prompt}": Lead with a punchy 7-word hero line, then 3 outcome-focused project cards.`,
  };
}
export const mockAnalytics = {
  views: Array.from({ length: 14 }, (_, i) => ({
    day: `D${i + 1}`,
    views: 200 + Math.round(Math.sin(i / 2) * 120 + Math.random() * 180),
  })),
  visitors: Array.from({ length: 14 }, (_, i) => ({
    day: `D${i + 1}`,
    visitors: 80 + Math.round(Math.cos(i / 3) * 60 + Math.random() * 90),
  })),
  devices: [
