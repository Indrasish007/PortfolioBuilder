...
function ChatbotTab() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      console.error("[HelpCenter Chat] API error:", err?.response?.status, err?.response?.data || err?.message);
      const errMsg =
        err?.response?.data?.error ||
        err?.response?.data?.detail ||
        "Sorry, I'm having trouble connecting right now. Please try again or use the Contact Support tab.";
      setMessages([
        ...history,
  ...
  return (
    <div className="flex flex-col h-[580px]">
      ...
      {messages.length > 2 && (
        <button
          onClick={() => {
            setMessages([{ role: "assistant", content: "Hi! 👋 I'm your PortfolioBuilder assistant. Ask me anything about the platform!" }]);
            setChipsUsed(false);
          }}
          className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 mb-2 self-end transition"
        >
            ...
            <GlassCard className="rounded-2xl p-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Powered by AI · Session only</p>
                </div>
